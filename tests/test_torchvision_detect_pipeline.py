...
def test_torchvision_detect_pipeline_images(test_images_path):
    """
    Test the TorchVisionDetectPipeline with image input.
    """
    weight = "fasterrcnn_resnet50_fpn"
    results_path = Path("results")
    pipeline = TorchVisionDetectPipeline(weight, preset, test_images_path, None, None, results_path)
...
def test_torchvision_detect_pipeline_videos(test_videos_path):
    """
    Test the TorchVisionDetectPipeline with video input.
    """
    weight = "fasterrcnn_resnet50_fpn"
    results_path = Path("results")
    pipeline = TorchVisionDetectPipeline(weight, preset, None, test_videos_path, None, results_path)
