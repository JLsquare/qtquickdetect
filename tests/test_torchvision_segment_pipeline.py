...
def test_torchvision_segment_pipeline_images(test_images_path):
    """
    Test the TorchVisionSegmentPipeline with image input.
    """
    weight = "maskrcnn_resnet50_fpn"
    results_path = Path("results")
    pipeline = TorchVisionSegmentPipeline(weight, preset, test_images_path, None, None, results_path)
...
def test_torchvision_segment_pipeline_videos(test_videos_path):
    """
    Test the TorchVisionSegmentPipeline with video input.
    """
    weight = "maskrcnn_resnet50_fpn"
    results_path = Path("results")
    pipeline = TorchVisionSegmentPipeline(weight, preset, None, test_videos_path, None, results_path)
