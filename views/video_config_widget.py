...
class VideoConfig(QWidget):
    ...
    def box_thickness_ui(self) -> QVBoxLayout:
        ...
        box_thickness_slider.setText(str(self._appstate.config.video_box_thickness))
    ...
    def text_size_ui(self) -> QVBoxLayout:
        ...
        text_size_slider.setText(str(self._appstate.config.video_text_size))
    ...
    def get_video_format(self) -> str:
        if self._appstate.config.video_format == 'mp4':
            return 'MP4 (.mp4)'
        elif self._appstate.config.video_format == 'avi':
            return 'AVI (.avi)'

    def set_video_format(self, value: str):
        if value == 'MP4 (.mp4)':
            self._appstate.config.video_format = 'mp4'
        elif value == 'AVI (.avi)':
            self._appstate.config.video_format = 'avi'
    ...
    def set_box_thickness(self, value: str):
        try:
            self._appstate.config.video_box_thickness = int(value)
            logging.debug('Set box thickness to {}'.format(value))
        except ValueError:
            self._appstate.config.video_box_thickness = 1
            logging.debug('Set box thickness to 1')

    def set_text_size(self, value: str):
        try:
            self._appstate.config.video_text_size = float(value)
            logging.debug('Set text size to {}'.format(value))
        except ValueError:
            self._appstate.config.video_text_size = 1.0
